...
const createRow = data => {
    ...
    iconEdit.addEventListener('click', updateSubjectRow);
...
const updateSubjectRow = async (e) => {
    const id = e.target.dataset.rowId;
    const subjects = await getAllSubjects();
    const subjectToUpdate = subjects.find(subject => subject.id === id);
    txtSubjectName.value = subjectToUpdate.name;

    btnSubjectSave.removeEventListener("click", insertSubjectItem);

    const updateSubjectItem = async () => {
        const newName = txtSubjectName.value;
        const updatedData = { id, name: newName };
        await updateSubject(id, updatedData);
        await bindSubjects();
        btnSubjectSave.removeEventListener("click", updateSubjectItem);
        btnSubjectSave.addEventListener("click", insertSubjectItem);

        txtSubjectName.value = '';
    };

    btnSubjectSave.addEventListener('click', updateSubjectItem);
};
